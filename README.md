...
Implement the handler so that the test now passes. DONE
...
Update the `createPayment` handler so that is generates its own unique ID when called, and returns that ID to the user when completed. DONE
...
Update `getPayment` so that if a matching payment is not found in the database, it returns a `HTTP 404` response to the caller. DONE
...
Update `createPayment` so that the request is validated before it's stored in the database. You should return a `HTTP 422` error to the caller if they provide you with data that isn't acceptable. DONE

## Question 5

The `listPayments` endpoint allows callers to get a list of all payments they've submitted. API users are now asking for the ability to search based on currency. E.g. they want to be able to find a list of all SGD payments submitted.
